    ...
    favoritesService
      ...
      .then((res) => {
        const match = res.data.find(
          (f) =>
            f.cityName.toLowerCase() === current.cityName.toLowerCase() &&
            f.country === current.country,
        );
        setFavoriteId(match?.id ?? null);
      })
      .catch(() => {});
  }, [current?.cityName, current?.country, isAuthenticated]);
  ...
  async function handleToggleFavorite() {
    ...
    try {
      if (favoriteId) {
        ...
        const res = await favoritesService.create({
    ...
    <div className="w-full max-w-2xl bg-gray-700/20 backdrop-blur-md rounded-3xl p-6 border border-white/10">
      <div className="flex items-start justify-between mb-4">
        <div>
          ...
          <h2 className="text-3xl font-bold text-white">{current.cityName}</h2>
        </div>
